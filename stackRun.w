...
%\footnote{\begin{minipage}{\textwidth}\verbatim{$Header: /mq/home4/m1gsa00/aim/stackSims/RCS/stackRun.w,v 1.6 2000/12/06 14:53:34 m1gsa00 Exp m1gsa00 $}\end{minipagez}}
}
...
\begin{quote}
  This paper provides an example calling the prototype stackSim() routine.
\end{quote}
...
program {\bf stackRun.o} with the nonlinear model {\bf julliard.o}
and the other {\bf stackProto} files
creating the program stochRun:
\begin{verbatim}
stackRun:	stackRun.o stackC.o  myNewt.o \
			ma50ad.o stackProto.o julliard.o
		f77 -o stackRun -O4 stackRun.o myNewt.o\
		stackC.o julliard.o ma50ad.o stackProto.o\
...
stackRun -a {\em t0} -s { \em stackasticPathLength} -r {\em replications }-l {\em pathLength}  -f {\em outputFilename}
\begin{description}
\item[{\bf t0}] The index of the first data value. This determines the initial
conditions for the set of simulations.
\item[{\bf stackasticPathLength}] The length of the stackastic path. Must be at least 1.
...
/*making a change in .w to see that it appears in .c*/
...
\subsection{Using the Q Matrix to Compute Stackastic Simulations}
\label{sec:qmatrix}
...
julMod,julModDerivative,julParam,julliardShocks,
fmats,fmatsj,fmatsi,
smats,smatsj,smatsi,
...
/* generateDraws(1,(stackasticPathLength),(1),SHOCKS,julliardPermVec);*/
...
fprintf(outFile,"julModRunParams={%d,%d,%d,%d,%d,%d,%d};\n",
     ...
     *pathLength,*t0,stackasticPathLength,1);
fPrintMathInt(outFile,1,failedQ,"julModFailedQ");
fPrintMathInt(outFile,1 * (stackasticPathLength),
      julliardPermVec,"julModPermVec");
fPrintMathDbl(outFile,(1 * julNEQS*(stackasticPathLength+julNLAGS)),
...
@{
   case 'a':
	 ...
	 break;
@}
@d command line stackasticPathLength
@{
   case 's':
	 stackasticPathLength=atoi(argv[2]);
     printf("got %d for stackasticPathLength\n",stackasticPathLength);
     if(stackasticPathLength<1)
	 {
       stackasticPathLength=1;
...
@<command line t0@>
@<command line stackasticPathLength@>
@<command line file@>
 default:
...
static char flnm[50] = "stackOut.m";
int i,j;
...
@{
failedQ=(int *)calloc(1,sizeof(int));
for(i=0;i<1;i++)failedQ[i]=0;
*tf=(*t0)+stackasticPathLength-1;

julliardPermVec=(int *)calloc(
     (stackasticPathLength)*(1),sizeof(int));
julliardPathQ=(double *)calloc(
    1*
    julNEQS*(julNLAGS+julNLEADS+(*pathLength)+stackasticPathLength),
sizeof(double));
...
@{
printf("$Id: stackRun.w,v 1.6 2000/12/06 14:53:34 m1gsa00 Exp m1gsa00 $\n");
